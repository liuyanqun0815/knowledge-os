...
def list_graph_entities(
    ...
    orchestrator = build_orchestrator_for_request(kb_id, request)
    return search_entities(orchestrator.deps.graph, q=q, predicate=predicate, limit=limit)
...
def list_graph_predicates(
    ...
    orchestrator = build_orchestrator_for_request(kb_id, request)
    return list_predicates(orchestrator.deps.graph, q=q, limit=limit)
...
def graph_entity_neighbors(
    ...
    edges = []
    for edge in graph.neighbors(entity_id, predicates=predicates, depth=depth):
